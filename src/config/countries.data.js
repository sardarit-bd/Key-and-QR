/**
 * ISO 3166-1 Alpha-2 Standard Worldwide Country Dataset
...
export const COUNTRIES = [
  ...
  { code: 'MK', name: 'Macedonia, Republic of', flag: '🇲🇰' },
...
/**
 * Get country object by ISO-2 code or name (case-insensitive)
 */
export const getCountryByCode = (code) => {
  ...
  return (
    COUNTRIES.find(
      (c) =>
        c.code === upperCode ||
        c.name.toLowerCase() === String(code).trim().toLowerCase()
    ) || null
  );
};

/**
 * Get formatted country display name by code
 */
...
/**
 * Multi-tiered scoring function for relevance ranking
 * - Score 100: Exact ISO-2 code match
 * - Score 80: Country name begins with query
 * - Score 60: ISO-2 code begins with query
 * - Score 40: Any word in country name begins with query
 * - Score 20: Substring contains match
 * - Score 0: No match
 */
export function getMatchScore(country, query) {
  ...
  if (code.startsWith(query)) return 60;

  const words = name.split(/\s+/);
  if (words.some((word) => word.startsWith(query))) return 40;

  if (name.includes(query) || code.includes(query)) return 20;

  return 0;
}

/**
 * Filter and rank countries based on relevance scoring with alphabetical fallback
 */
export function filterAndRankCountries(countries = COUNTRIES, rawQuery = '') {
  ...
  return countries
    .map((country) => ({
      country,
      score: getMatchScore(country, query),
    }))
    ...
    .sort((a, b) => {
      ...
      return a.country.name.localeCompare(b.country.name); // Alphabetical fallback
    })
    .map(({ country }) => country);
}
