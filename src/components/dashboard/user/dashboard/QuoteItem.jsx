...
export default function QuoteItem({ quote }) {
  ...
  return (
    <div className="flex items-center gap-3 sm:gap-4 p-2.5 sm:p-3 md:p-3.5 rounded-xl bg-background-secondary border border-border hover:bg-muted transition-colors group">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-foreground text-xs sm:text-sm font-medium truncate">{title}</p>
      </div>
      
      <div className="flex items-center gap-2 sm:gap-3 md:gap-4 flex-shrink-0">
        <div className="hidden sm:flex items-center gap-1.5 px-2 md:px-2.5 py-0.5 md:py-1 rounded-md bg-muted text-[10px] md:text-[11px] font-medium border border-border">
          <BadgeIcon size={10} className={`w-[8px] h-[8px] md:w-[10px] md:h-[10px] ${badgeColor}`} />
          <span className={badgeColor}>{category}</span>
        </div>
        
        <div className="flex items-center gap-1 text-muted-foreground text-[10px] sm:text-[11px] min-w-[70px] sm:min-w-[90px] md:min-w-[100px] justify-end">
