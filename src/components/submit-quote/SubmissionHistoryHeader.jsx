...
export default function SubmissionHistoryHeader() {
  return (
    <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-5 w-full max-w-full overflow-hidden sm:overflow-visible">
      ...
      <div className="relative min-w-0 max-w-full">
        <div className="flex items-center gap-3 sm:gap-3.5">
          ...
          <div className="relative flex h-11 w-11 shrink-0 items-center justify-center sm:h-14 sm:w-14">
            ...
            <History
              className="relative h-5 w-5 sm:h-6 sm:w-6 text-primary dark:text-violet-300"
              strokeWidth={1.8}
            />
          </div>
      ...
      <Link href="/new-dashboard/user/submit-quote" className="w-full sm:w-auto shrink-0">
        <button
          type="button"
          className="group relative inline-flex w-full sm:w-auto cursor-pointer items-center justify-center gap-2 overflow-hidden rounded-full border border-accent/25 bg-gradient-to-r from-accent/15 to-accent/5 px-4 py-2.5 sm:py-2 text-[13px] font-medium text-accent shadow-[0_8px_24px_-8px_rgba(253,182,92,0.3)] backdrop-blur-md transition-all duration-300 hover:-translate-y-0.5 hover:border-accent/40 hover:shadow-[0_12px_32px_-8px_rgba(253,182,92,0.4)] active:translate-y-0 active:scale-[0.98]"
        >
